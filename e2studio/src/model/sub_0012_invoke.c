...
__attribute__((aligned(32), section(".sdram_noinit_nocache"))) uint8_t sub_0012_arena[49152];
// Fast scratch arena not used for Ethos-U55
//  We will not create it for now and reuse the address of the other arena
// __attribute__((aligned(16))) static uint8_t sub_0012_fast_scratch[49152];
...
int sub_0012_invoke(bool clean_outputs) {
  ...
  // Buffer sub_0012_arena with size 49152 and address: 0
  base_addrs[1] = (uint64_t)(uintptr_t) (sub_0012_arena+0);
  base_addrs_size[1] = 49152;

  // Buffer sub_0012_fast_scratch with size 49152 and address: 0
  base_addrs[2] = (uint64_t)(uintptr_t) (sub_0012_arena+0);
  base_addrs_size[2] = 49152;

  // Buffer input_tensor_0 with size 12288 and address: 8192
  base_addrs[3] = (uint64_t)(uintptr_t) (sub_0012_arena+8192);
  base_addrs_size[3] = 12288;

  // Buffer output_tensor_0 with size 8192 and address: 0
  if (clean_outputs) {
    memset(sub_0012_arena + 0, 0, 8192);
  }
  base_addrs[4] = (uint64_t)(uintptr_t) (sub_0012_arena+0);
  base_addrs_size[4] = 8192;

  // Buffer output_tensor_1 with size 32768 and address: 16384
  if (clean_outputs) {
    memset(sub_0012_arena + 16384, 0, 32768);
  }
  base_addrs[5] = (uint64_t)(uintptr_t) (sub_0012_arena+16384);
