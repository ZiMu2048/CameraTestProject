...
fsp_err_t glcdc_init(void)
{
    ...
    APP_ERR_RET( FSP_SUCCESS != err, err, "** R_GLCDC_Open API FAILED **");
     ...
     /* Update the runtime configuration for the graphics1 layer */
     ...
     /* Update the buffer address for the graphics1 layer */
     g_display_runtime_cfg_bg.input.p_base = (uint32_t *) &fb_background[0];
